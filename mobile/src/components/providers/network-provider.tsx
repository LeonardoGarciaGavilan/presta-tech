...
export function NetworkProvider({ children }: { children: React.ReactNode }) {
  ...
  const staleEvaluatedRef = useRef(false);

  const refreshStats = useCallback(async () => {
    const stats = await getQueueStats();
    setPendingCount(stats.pending);
    setFailedCount(stats.failed);
    setExpiredCount(stats.expired);
  }, []);
  ...
  useEffect(() => {
    if (!bootSyncDoneRef.current && network.isOnline) {
      ...
      staleEvaluatedRef.current = true;
      (async () => {
  ...
  useEffect(() => {
    ...
    if (!network.isOnline) {
      ...
      staleEvaluatedRef.current = false;
    }
  }, [network.isOnline, pendingCount, triggerSync]);

  useEffect(() => {
    const unsubscribe = onOnline(() => {
      // Primero evaluar operaciones stale al reconectar
      (async () => {
        if (!staleEvaluatedRef.current) {
          staleEvaluatedRef.current = true;
          await recoverSyncingItems();
          await markStaleAsFailed();
        }
        ...
        const stats = await getQueueStats();
        if (stats.pending > 0 || stats.expired > 0) {
          await triggerSync();
        }
  ...
  // Escuchar operaciones expiradas para mostrar notificación
  useEffect(() => {
    const unsubscribe = onStaleExpired((count) => {
      refreshStats();
      // Mostrar notificación visible al usuario
      if (count > 0) {
        showToast(
          `${count} operación${count > 1 ? 'es' : ''} financiera${count > 1 ? 's' : ''} requieren revisión manual`,
          'info',
          8000,
        );
      }
      if (__DEV__) {
        console.log(`[NetworkProvider] ${count} operación(es) financiera(s) expirada(s) - requieren revisión`);
      }
    });
    return unsubscribe;
